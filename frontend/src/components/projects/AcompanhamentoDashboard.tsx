...
export function AcompanhamentoDashboard({ isManager = false }: { isManager?: boolean }) {
        ...
        {managed ? (
          <div className="acp-manage">
            ...
            <div className="acp-manage-body">
              <ProjectScheduleEditor key={managed.projectId} ref={scheduleRef} projectId={managed.projectId} canManage={isManager} onDirtyChange={setManagedDirty} />
            </div>
            <div className="acp-manage-foot">
              <button type="button" className="mini-btn alt" onClick={() => setManaged(null)}>Cancelar</button>
              {isManager ? <button type="button" className="mini-btn" disabled={!managedDirty} onClick={() => scheduleRef.current?.save()}>Salvar</button> : null}
            </div>
          </div>
        ) : <div />}
      </Modal>
