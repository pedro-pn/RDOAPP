...
export function MissionsBoard({ canManage, planId, status, search, selectedMissionId, onSearchChange, onStatusChange, onMissionSelect }: {
  ...
  return (
    <div className="efetivo-board" data-efetivo-missions>
      ...
      {canManage ? <MissionFormModal open={formOpen} mission={editing} planId={planId} projects={projects.data || []} roles={roles.data || []} collaborators={collaborators.data || []} saving={save.isPending} onClose={() => { setFormOpen(false); setEditing(null); }} onSubmit={payload => save.mutate(payload)} /> : null}
