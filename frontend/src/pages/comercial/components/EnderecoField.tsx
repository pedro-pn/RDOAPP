 ...
 * reconhece. E endereço da sede é a **origem** de toda distância calculada — o
 * lugar onde um erro não aparece, porque quem confere um cálculo olha o destino.
 *
 * Escolher da lista traz o `placeId` junto, e é isso que o servidor grava: o
 * identificador do lugar, não o texto. Digitar à mão continua valendo, e aí o
 * servidor geocodifica como antes — a lista **sugere**, não obriga.
 *
 * ---------------------------------------------------------------------------
...
export function EnderecoField({
  ...
  useEffect(() => {
    ...
    const relogio = window.setTimeout(() => {
      ...
      sugerirEnderecos(termo, controle.signal)
        ...
        .finally(() => {
          if (vez.current === minhaVez) setBuscando(false);
        });
    }, ESPERA_MS);
  ...
  function digitar(texto: string) {
    setTermo(texto);
    // O `placeId` anterior era de OUTRO endereço. Mantê-lo faria o servidor
    // gravar o lugar antigo com o texto novo.
    onChange(texto, '');
  }
    ...
    <div className={invalid ? 'field-group field-invalid' : 'field-group'}>
      ...
      <div className="com-autocomplete">
        ...
        {aberta && sugestoes.length > 0 && (
          <ul className="com-autocomplete-lista" id={listaId} role="listbox">
            {sugestoes.map((sugestao, indice) => (
              <li
                ...
                onMouseDown={evento => {
                  // `mouseDown`, não `click`: o `blur` do campo chega primeiro e
                  // o clique nunca aconteceria.
                  evento.preventDefault();
                  escolher(sugestao);
                }}
              ...
              >
                ...
                {sugestao.secundario && (
                  <span className="com-quebrar">{sugestao.secundario}</span>
                )}
              </li>
            ))}
          </ul>
