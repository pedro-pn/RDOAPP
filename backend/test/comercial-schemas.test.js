...
test("a criação de levantamento NÃO aceita totais do cliente", () => {
  ...
  assert.ok(
    !("marginPercent" in parsed),
    "marginPercent não pode entrar pelo corpo",
  );
});
...
test("levantamento aceita rascunho explícito e mantém SALVO como padrão compatível", () => {
  ...
  assert.equal(
    schemas.costEstimateCreate.safeParse({ ...entrada, status: "QUALQUER" })
      .success,
    false,
  );
});
...
test("o filtro de arquivados interpreta 0 e false como falso", () => {
  for (const arquivados of [undefined, false, 0, "0", "false"]) {
    ...
    assert.equal(
      schemas.proposalListQuery.parse({ arquivados }).arquivados,
      false,
    );
  }
...
test("as listagens normalizam paginação e limitam o tamanho da página", () => {
  assert.deepEqual(schemas.listQuery.parse({}), {
    arquivados: false,
    busca: "",
    page: 1,
    pageSize: 25,
  ...
  assert.equal(
    schemas.proposalListQuery.parse({ page: "3", pageSize: "50" }).page,
    3,
  );
  assert.equal(
    schemas.proposalListQuery.parse({ page: "3", pageSize: "50" }).pageSize,
    50,
  );
  assert.equal(schemas.listQuery.safeParse({ pageSize: 101 }).success, false);
  assert.equal(
    schemas.listQuery.safeParse({ status: "FINALIZADA" }).success,
    false,
  );
});
...
test("os dois PUTs exigem a versão carregada e aceitam sobrescrita confirmada", () => {
  ...
  assert.equal(
    schemas.proposalUpdate.safeParse({ clientName: "X" }).success,
    false,
  );
  assert.equal(
  ...
  assert.equal(
    schemas.costEstimateUpdate.safeParse(levantamento).success,
    true,
  );
  assert.equal(
    schemas.costEstimateUpdate.safeParse({
      ...levantamento,
