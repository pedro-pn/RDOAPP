...
router.put('/:id/manual-data', requireAuth, requireRdoManager, asyncHandler(async (req, res) => {
  const data = manualReportOperationalDataSchema.parse(req.body || {});
  ...
  if (!manualReportUploadMeta(existing).uploadedAt) {
    return res.status(400).json({ error: 'Apenas relatórios enviados manualmente podem ter os dados operacionais editados por este fluxo.' });
  }
