...
- [X] T003 [P] Criar `backend/src/lib/equipamentos/equipment-checklist.js`: `normalizeChecklistItems(value)` (trim, remove vazios, max 300 chars/item, max 100 itens, preserva ordem) e `resolveEffectiveChecklist(equipment, category)` (`equipment.checklistItems ?? category.checklistItems`, somente se `category.checklistEnabled`; retorna `[]` caso contrário).
...
- [X] T006 [US1] Backend: criar `backend/src/lib/romaneio/romaneio-checklist-docx.js` — carrega `Modelos/definitivos/Checklist.docx`, substitui `<<projeto>>` (`buildChecklistProjectLabel`: `<código> - <nome>`, só código se missão sem nome), `<<data>>`, `<<categoria>>`, `<<nomeoutag>>`, clona linha-template `<<item>>`/`<<status>>` por ponto, aplica cor no run do status (`w:rPr/w:color` `00B050`/`FF0000`/`808080`), `buildChecklistFileName(romaneio)` (`Checklist - Missão <código> - <dd-mm-yyyy>.pdf`, com `safePath`), salva DOCX temporário na pasta da missão/ROMANEIO, converte via `convertDocxToPdf` e remove o DOCX. Padrões: `romaneio-docx.js` (tokens/clonagem).
...
- [X] T032 [US1] Backend/lib: atualizar `backend/src/lib/equipamentos/equipment-checklist.js` com `resolveChecklistDisplayName`/`resolveChecklistCategoryName`, aplicando `AUTO` (tag/código para equipamentos/itens por unidade; nome para consumíveis/produtos) e overrides `TAG`/`NAME`.
- [X] T033 [US1] Backend/docx: atualizar `backend/src/lib/romaneio/romaneio-checklist-docx.js` para gerar 1 DOCX/PDF consolidado por romaneio; localizar e clonar a tabela com `<<categoria>>`, `<<nomeoutag>>`, `<<item>>`, `<<status>>` para cada snapshot; clonar a linha-template de pontos dentro de cada tabela; manter status colorido e assinatura final única.
